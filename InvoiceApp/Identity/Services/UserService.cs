...
namespace InvoiceApp.Identity.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<PagedList<AppUser>> Get(UserRequestParameters parameters)
        {
            ...
            {
                users = users.Where(i => EF.Functions.Like(i.Name, $"{parameters.Name}%"));
            }

            if (!string.IsNullOrEmpty(parameters.Surname))
            {
                users = users.Where(i => EF.Functions.Like(i.Surname, $"{parameters.Surname}%"));
            }

            if (!string.IsNullOrEmpty(parameters.Email))
            {
                users = users.Where(i => EF.Functions.Like(i.Email, $"{parameters.Email}%"));
            }
            ...
            var source = await users.ToArrayAsync();

            return new PagedList<AppUser>(source, parameters.Page, parameters.PageSize, totalCount);
        }
